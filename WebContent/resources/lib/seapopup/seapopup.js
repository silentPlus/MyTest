...
(function (D) {
	...
	B.fn = B.prototype = {
		seaprepend : function () {
			...
			var H = O == "" ? "" : '<div class="seaPopupCloseBtn85js93" style="position:absolute;height:28px;line-height:28px;width:50px;top:12px;right:10px;color:#868686;font-weight:normal;' + K + '">' + O + "</div>";
			var Z = '<div class="seaPopupBgDiv847fd8" style="position:fixed;width:100%;height:100%;top:0;left:0; background-color:#666; opacity:0.5; z-index:' + (1000 + A.length) + ";" + Y + '"></div><div class="seaPopupBox39fj7d" style="position:fixed;display:none;left:0;' + (U == "center" ? "top:50%;left:-80%;width:80%;border-radius:5px;" : U == "bottom" ? "bottom:-100%;left:0;width:100%;" : "top:-100%;left:0;width:100%;") + "z-index:" + (1001 + A.length) + ";color:#454545;background-color:#fff;" + L + '">' + (M == "" ? "" : '<div class="seaPopupTitle384jx8" style="padding:15px 10px;line-height:26px;text-align:center;font-weight:bold;font-size:1.5em;position:relative;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;' + R + '">' + M + H + "</div>") + '<div class="seaPopupContent98fd3" style="width:100%;line-height:22px;text-align:center;">' + X + "</div>";
			if (!F && U == "center") {
...
(function (A) {
	A.fn.seaSelectDlg = function (B, C) {
		...
		E.unbind("click").bind("click", function () {
			...
			for (var M in B) {
				var J = B[M];
				K += '<li data-value="' + J["value"] + '" data-text="' + J["text"] + '" style="line-height:22px; padding:12px 0 12px;font-size:0.9375em; color:#454545; border-bottom:1px solid #eceaea;text-align:center;position:relative;' + I.liStyle + (D == J["value"] ? (!I.selectedImgUrl ? "color:#00f;font-weight:bold;" : "") + I.selectedStyle : "") + '">' + J["text"] + (D == J["value"] ? H : "") + "</li>"
			}
			K += "</ul>";
			I = A.extend({}, {
					message : K
...
function alertDlg(B, A) {
	return $.seaPopup({
		title : "提示",
		message : '<div style="margin:0 3%;font-size:1.2em;word-break: break-all;">' + B + "</div>",
		cancelText : "确定",
		cancelCallback : A || function () {},
		cancelOnly : true
	})
}
function confirmDlg(C, B, A) {
	return $.seaPopup({
		title : "提示",
		message : '<div style="margin:0 3%;font-size:1.2em;word-break: break-all;">' + C + "</div>",
		doneText : "确定",
		doneCallback : B || function () {},
		cancelCallback : A || function () {}
	...
	$.fn.datePopup = function(MZ){
		...
		$(this).on("click",function(event){
			...
			if( currentDateStr != "" ){
				...
				// 当$(this)值为空时的初始弹窗值
				initPopupDate = initPopupDate?initPopupDate.getFullYear() + "-" + returnTwoNum(initPopupDate.getMonth()) + "-" + returnTwoNum(initPopupDate.getDate()):'';
				currentDateStr = initPopupDate || startYear+ "-01-01";
				currentDateArr = currentDateStr.split("-");
			}
			...
			var popUp = $.seaPopup({
				...
				closeText:"<span style=\"color:#828282;border:1px solid #828282;border-radius:3px;font-size:14px;display:block;\">完成</span>",
				closeCallBack:function(){
					//$(self).val("");
